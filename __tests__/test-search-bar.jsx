...
var TestView = React.createClass({
  ...
  toggleColumn: function(column) {
    ...
    column_index != -1
      ? hidden_columns.splice(column_index, 1)
      : hidden_columns.push(column.props.getAttr());
      ...
      <div>
        ...
        <Table data={[]} ref="table" hiddenColumns={this.state.hiddenColumns}>
          ...
          <Table.Column
            ...
            attr="category"
            filter={true}
            filter-settings={this.getCategoryFilterSettings()}
            data-type="date"
          />
        </Table>
      </div>
...
describe("SearchBar", () => {
  ...
  it("builds filter queries correctly", () => {
    ...
    searchBarComponent.setDaterange(2, {
      start: moment("01012001", "DDMMYYYY"),
      end: moment("02022001", "DDMMYYYY"),
    });
    searchBarComponent.setValue(3, null, "begginer");

    // Check Queries
    expect(searchBarComponent.getQuery()).toEqual({
      q: undefined,
      name: "Jon",
      number: ">3",
      birthDate: "2001-01-01to2001-02-02",
      category: "begginer",
    });
    expect(searchBarComponent.getHTQuery()).toBe(
      "name~'Jon'&number > 3&between(date(birthDate), '2001-01-01', '2001-02-02')&category=='begginer'"
    );

    // Deactivate filters
    TestUtils.Simulate.click(searchBarComponent.refs.nameFilterRemoveButton);
    TestUtils.Simulate.click(
      searchBarComponent.refs.numberFilter.children[0].children[3].children[0]
    );
    TestUtils.Simulate.click(
      searchBarComponent.refs.birthDateFilter.children[0].children[4].children[0]
    );
  ...
  it("handles numeric filter correctly", () => {
    ...
    // Close filter
    TestUtils.Simulate.click(
      searchBarComponent.refs.numberFilter.children[0].children[5].children[0]
    );
  });
});
