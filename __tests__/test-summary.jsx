...
var summaryComponent = TestUtils.renderIntoDocument(
  <Summary data={summaryData}>
    <Summary.Item
      ...
      attr="total"
      htsql="sum(sale.cost)"
      formatter={Utils.formatters.currency}
      icon="fa fa-database"
      color="bg-green"
    />
    <Summary.Item
      ...
      attr="quantity"
      htsql="count(sale)"
      formatter={Utils.formatters.numeric}
      sublabel="Items Quantity"
      subhtsql="count(sale.items)"
      subformatter={Utils.formatters.numeric}
      icon="fa fa-dollar"
      color="bg-aqua"
    />
    <div />
...
describe("Summary", () => {
  it("builds htsql query correctly", () => {
    ...
    expect(htsql).toBe(
      "total:=sum(sale.cost),quantity:=count(sale),subquantity:=count(sale.items)"
    );
  });

  it("render correct and formatted data", () => {
    var summaryNumberBoxes = TestUtils.scryRenderedDOMComponentsWithClass(
      summaryComponent,
      "info-box-number"
    );
    ...
    expect(summaryNumberBoxes[2].textContent).toBe(
      Utils.formatters.numeric(summaryData.subquantity)
    );
  });

  it("ignores non summary item children", () => {
    var summaryInfoBoxes = TestUtils.scryRenderedDOMComponentsWithClass(
      summaryComponent,
      "info-box"
    );
