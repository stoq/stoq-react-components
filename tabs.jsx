...
let Tabs = React.createClass({
  ...
  _getTab: function(ids, tab, index) {
    if (!tab || tab.type !== Tabs.Tab) return;
    return (
      <div key={index} id={ids[index]} className={"tab-pane " + (tab.props.className || "")}>
        {tab.props.children}
      </div>
    );
  },
  ...
  render: function() {
    ...
    return (
      <div className={this.props.customHeader ? "nav-tabs-custom" : ""}>
        <ul className="nav nav-tabs">
          {this.props.children.map(function(tab, index) {
            ...
            if (tab.type == Tabs.Config) {
              return (
                <li key={index} className="pull-right dropdown">
            ...
            return (
              <li key={index} className={tab.props.className} onClick={this.click.bind(this, tab)}>
